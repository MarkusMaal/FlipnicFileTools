...
public abstract class Converter
{
    ...
    public static void SfxBankToSoundFont2(string hdFilePath, string bdFilePath, string outputFilePath, bool synthesizeWav = false, bool fakeSustainR = true)
    {
        // combine .hd/.bd files
        ...
        var dir = new FileInfo(outputFilePath).DirectoryName;
        var noExtName = Path.GetFileNameWithoutExtension(outputFilePath);
        foreach (var (i, seSeq) in instrument.SoundEffectSequences.Index())
        {
            seSeq.ToMidi(Path.Combine(dir ?? ".", noExtName + "." + (i+1) + ".MID"));
        }

        // channel = program in this case
        for (var j = (byte)0; j < instrument.SeProgramChunks.Count; j++)
        {
            channelToPrograms.Add((j, j));
        }
        ...
        {
            if (channelToPrograms[j].Program >= instrument.SeProgramChunks.Count) continue;
            var prog = instrument.SeProgramChunks[channelToPrograms[j].Program];
        ...
        for (var j = 0; j < channelToPrograms.Count; j++)
        {
            ...
            for (var k = 0; k < prog.SplitChunks.Count; k++)
            {
                ...
                if (vol == 0)
                {
                    vol = Math.Max(prog.BaseVolume, splitChunk.Volume);
                }
                if (fakeSustainR)
                {
                ...
                sf2.AddInstrumentGenerator(SF2Generator.KeyRange,
                    new SF2GeneratorAmount
                        {
                            LowByte = (byte)(prog.SplitChunks[k].BaseNote), HighByte = (byte)prog.SplitChunks[k].BaseNote
                        });
