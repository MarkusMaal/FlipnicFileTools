...
public class SaveIcon(byte [] data)
{
    public int IconFileId { get; init; } = StaticUtils.GetInt32(data, 0);
    public int AnimationShapes { get; init; } = StaticUtils.GetInt32(data, 4);
    public int TextureType { get; init; } = StaticUtils.GetInt32(data, 8);
    public int VertexCount { get; init; } = StaticUtils.GetInt32(data, 16);
    ...
    public void Read()
    {
        ...
        var frames = StaticUtils.GetInt32(data, animOffset+16);
        var skip = 0x14;
        for (var i = 0; i < frames; i++)
        {
            var numKeys = StaticUtils.GetInt32(data, animOffset+skip+4);
            skip += 8 + numKeys * 8;
        }
...
{
    public short CoordX { get; init; } = StaticUtils.GetInt16(vertexData, 0);
